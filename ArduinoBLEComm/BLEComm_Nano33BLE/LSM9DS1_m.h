...
#define LSM9DS1_ACCEL_SCALE_G      2.0
#define LSM9DS1_ACCEL_FACTOR       32768.0

class LSM9DS1_m_Class {
  public:                      //public은 구조체 또는 클래스 외부에서 접근할 수 있는 구조체 또는 클래스의 공개함수
    LSM9DS1_m_Class(TwoWire& wire);
    virtual ~LSM9DS1_m_Class();
    ...
    // Accelerometer
    virtual int readAcceleration(float& x, float& y, float& z); // Results are in G (earth gravity). virtual -> 가상함수
    virtual int readAcceleration(int16_t data[3]); // Results are in int16 (earth gravity). virtual -> 가상함수
